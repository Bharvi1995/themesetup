...
class MerchantRefundsTransactionmanagementCSVExport
{
    protected $id;
    ...
    public function download(Request $request)
    {
        ...
        return response()->streamDownload(function () use ($columns, $input) {
            ...
            $data = Transaction::select(
                ...
                ->join('middetails', 'middetails.id', 'transactions.payment_gateway_id')->where('transactions.chargebacks', '0')->where('transactions.refund', '1')
            ...
            if ((isset($input['refund_start_date']) && $input['refund_start_date'] != '') && (isset($input['refund_end_date']) && $input['refund_end_date'] != '')) {
                $refund_start_date = date('Y-m-d', strtotime($input['refund_start_date']));
                $refund_end_date = date('Y-m-d', strtotime($input['refund_end_date']));

                $data = $data->where(DB::raw('DATE(transactions.refund_date)'), '>=', $refund_start_date . ' 00:00:00')
                    ->where(DB::raw('DATE(transactions.refund_date)'), '<=', $refund_end_date . ' 00:00:00');
            } else if ((isset($input['refund_start_date']) && $input['refund_start_date'] != '') || (isset($input['refund_end_date']) && $input['refund_end_date'] == '')) {
                $refund_start_date = date('Y-m-d', strtotime($input['refund_start_date']));
                $data = $data->where(DB::raw('DATE(transactions.refund_date)'), '>=', $refund_start_date . ' 00:00:00');
            } else if ((isset($input['refund_start_date']) && $input['refund_start_date'] == '') || (isset($input['refund_end_date']) && $input['refund_end_date'] != '')) {
                $refund_end_date = date('Y-m-d', strtotime($input['refund_end_date']));
                $data = $data->where(DB::raw('DATE(transactions.refund_date)'), '<=', $refund_end_date . ' 00:00:00');
            }
