...
class MerchantChargebacksTransactionmanagementCSVExport
{
    protected $id;
    ...
    public function download(Request $request)
    {
        ...
        return response()->streamDownload(function () use ($columns, $input) {
            ...
            $data = Transaction::select(
                ...
                ->join('middetails', 'middetails.id', 'transactions.payment_gateway_id')->where('transactions.chargebacks', '1')
            ...
            if ((isset($input['chargebacks_start_date']) && $input['chargebacks_start_date'] != '') && (isset($input['chargebacks_end_date']) && $input['chargebacks_end_date'] != '')) {
                $chargebacks_start_date = date('Y-m-d', strtotime($input['chargebacks_start_date']));
                $chargebacks_end_date = date('Y-m-d', strtotime($input['chargebacks_end_date']));

                $data = $data->where(DB::raw('DATE(transactions.chargebacks_date)'), '>=', $chargebacks_start_date . ' 00:00:00')
                    ->where(DB::raw('DATE(transactions.chargebacks_date)'), '<=', $chargebacks_end_date . ' 00:00:00');
                ...
                $chargebacks_end_date = date('Y-m-d', strtotime($input['chargebacks_end_date']));
                $data = $data->where(DB::raw('DATE(transactions.chargebacks_date)'), '<=', $chargebacks_end_date . ' 00:00:00');
            }
            ...
            fclose($file);
        }, 'Chargebacks_Transaction_Excel_' . date('d-m-Y') . '.csv');
    }
}