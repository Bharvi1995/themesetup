...
class MerchantRetrievalTransactionmanagementCSVExport
{
    protected $id;
    ...
    public function download(Request $request)
    {
        ...
        $data = Transaction::select('transactions.order_id','transactions.customer_order_id','applications.business_name',
                    ...
                    ->join('middetails', 'middetails.id', 'transactions.payment_gateway_id')->where('transactions.chargebacks', '0')->where('transactions.is_retrieval', '1')
            ...
            if ((isset($input['retrieval_start_date']) && $input['retrieval_start_date'] != '') && (isset($input['retrieval_end_date']) && $input['retrieval_end_date'] != '')) {
                $retrieval_start_date = date('Y-m-d', strtotime($input['retrieval_start_date']));
                $retrieval_end_date = date('Y-m-d', strtotime($input['retrieval_end_date']));
                
                $data = $data->where(DB::raw('DATE(transactions.retrieval_date)'), '>=', $retrieval_start_date . ' 00:00:00')
                ->where(DB::raw('DATE(transactions.retrieval_date)'), '<=', $retrieval_end_date . ' 00:00:00');
                ...
                $retrieval_end_date = date('Y-m-d', strtotime($input['retrieval_end_date']));
                $data = $data->where(DB::raw('DATE(transactions.retrieval_date)'), '<=', $retrieval_end_date . ' 00:00:00');
            }
            ...
            fclose($file);
        }, 'Retrieval_Transaction_Excel_'.date('d-m-Y').'.csv');     
    }
}
