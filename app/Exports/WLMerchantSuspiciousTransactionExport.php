...
class WLMerchantSuspiciousTransactionExport
{
    ...
    public function download()
    {
        ...
        return response()->streamDownload(function () use ($columns, $input, $payment_gateway_id) {
            ...
            $data = Transaction::select('transactions.*', DB::raw("(DATE_FORMAT(transactions.created_at, '%d-%m-%Y %h:%i:%s')) AS created_at_c"), DB::raw("(DATE_FORMAT(transactions.chargebacks_date, '%d-%m-%Y %h:%i:%s')) AS chb_date"))
            ...
            fclose($file);
        }, 'Suspicious_Transcation_Excel_' . date('d-m-Y') . '.csv');
    }
