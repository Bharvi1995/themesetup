...
class AgentAllChargebacksTransactionsExport implements FromCollection, WithHeadings, WithMapping
{
    /**
    ...
    */
    public function collection()
    {
        ...
        if((isset($input['start_date']) && $input['start_date'] != '') &&  (isset($input['end_date']) && $input['end_date'] != '')) {
            ...
            $data = $data->where(DB::raw('DATE(transactions.changebanks_date)'), '>=', $start_date.' 00:00:00')
                ->where(DB::raw('DATE(transactions.changebanks_date)'), '<=', $end_date.' 00:00:00');
        }

		$data = $data->where('transactions.chargebacks', '1')
            ->whereIn('transactions.user_id',$userIds)
            ->orderBy('transactions.changebanks_date', 'DESC');
