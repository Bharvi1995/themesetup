...
class QikPay extends Controller
{
    use StoreTransaction;

    const BASE_URL = 'https://secure.qikpay.co.in/pgui/jsp/paymentrequest'; // live
    // const BASE_URL = 'https://sandbox.qikpay.co.in/pgui/jsp/paymentrequest'; // test
  
    public function checkout($input, $check_assign_mid) {
        
        try {
            $currency = config('qikpay.currency.' . strtoupper($input['converted_currency']));
            if (empty($currency)) {
            ...
            return [
                ...
                'redirect_3ds_url' => route('qikpay-confirmation', [Crypt::encryptString($input['session_id'])]),
            ];  
        } catch(\Exception $e) {
            \Log::info([
                'qikpay-exception' => $e->getMessage(),
                'id' => $input['session_id']
            ]);
            return [
                'status' => '0',
    ...
    public function confirmation(Request $request, $session_id) {
        ...
        try {
            ...
            $data = [
                ...
                'RETURN_URL' => route('qikpay-callback',$input['session_id'])
            ]; 
            ksort($data);
            $explodeArray = urldecode(http_build_query($data, '', '~'));
            $explodeArray = $explodeArray .$check_assign_mid->secret_key;
            $url = self::BASE_URL;
            \Log::info([
                'qikpay-form-HASH' => $explodeArray,
                'id' => $input['session_id']
            ]);
            ...
            $error = $e->getMessage();
            \Log::info([
                'qikpay-form-exception' => $e->getMessage(),
                'id' => $session_id
            ]);
            $session_id = $data = '' ;
        }
        return view('gateway.qikpay', compact('error', 'data', 'session_id'));
    }
